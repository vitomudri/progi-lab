<template>
    ...
    <section class="popular">
        ...
        <div class="recipe-grid">
            ...
            <router-link v-for="recipe in recipes" :key="recipe.recipe_id" :to="`/recipe/${recipe.recipe_id}`" class="recipe-card">
                <!-- <img :src="recipe.image" alt="" class="thumb" /> -->
                <p>{{ recipe.name }}</p>
            </router-link>

            <!-- <div v-for="n in (8 - recipes.length)" :key="'placeholder-' + n" class="recipe-card placeholder">
                <div class="thumb empty"></div>
                <p>Recept...</p>
            </div> -->

            <router-link v-if="isLoggedIn" to="/add-recipe" class="recipe-card add-card">
...
type RecipeSummary = { recipe_id: string; name: string; }
...
onMounted(async () => {
    ...
    try {
        const res = await fetch("/api/v1/recipe/popular");
